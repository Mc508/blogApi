...
def CreatePost(request):
    ...
    if serializer.is_valid():
        serializer.save()
        return Response({"success":"the post is successfully created"},status=201)
    else:
...
def DeletePost(request):
    ...
    try:
        ...
        post.delete()
        return Response({"success":"the post is successfully deleted successfully"},status=200)
...
def UpdatePost(request):
    ...
    try:
        ...
        post.save()
        return Response({"Success":"The post is successfully updated "},status=200)
    except Post.DoesNotExist:
        return Response({"Error":"The post does not exist"},status=404)